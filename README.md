...
- **SQLite (In-Memory / File):** Does _not_ support row-level locking. Under high write concurrency, SQLite will lock the entire database, leading to database lock errors rather than serialized queuing. Therefore, testing or deploying concurrent workloads against standard SQLite is unsupported.
...
The solver navigates the player from their _current_ position `X` using the following sequence of steps:
...
_Note: If the player successfully lands on `$`, they win the game and the session is cleared._
