...
test('it accepts moves with zero steps', function () {
    ...
    // Go Up 3, Right 4, Down 0, Left 0 -> lands on (1, 5)
    $this->artisan('play:hidden-item', ['--A' => 3, '--B' => 4, '--C' => 0, '--D' => 0])
        ->assertExitCode(0)
        ->expectsOutput('Player moved to: (Row 1, Col 5)')
        ->expectsOutput('########')
        ->expectsOutput('#....X.#') // Player X is at (1, 5)
        ->expectsOutput('#.###$.#') // Item $ is at (2, 5)
        ->expectsOutput('#...#.##')
        ->expectsOutput('#.#....#') // Starting spot becomes empty '.'
        ->expectsOutput('########');
...
test('it blocks moves with negative steps', function () {
    ...
    $this->artisan('play:hidden-item', ['--A' => -1, '--B' => 2, '--C' => 0, '--D' => 0])
        ->assertExitCode(1)
